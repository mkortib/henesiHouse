    ...
    <div class="w">
        <section class="top flex">
            ...
            <div class="filters cnt flex flex-sb">
                ...
                <div class="r_bl">
                    ...
                    <form>
                        <input type="number" name="min_square" placeholder="<?php echo __('[:ua]від[:ru]от[:en]from[:]'); ?>" value="<?php if (isset($_GET['min_square'])) { echo $_GET['min_square']; } ?>">
                        <input type="number" name="max_square" placeholder="<?php echo __('[:ua]до[:ru]до[:en]to[:]'); ?>" value="<?php if (isset($_GET['max_square'])) { echo $_GET['max_square']; } ?>">
                        <div class="wrapper">
                ...
                if ( have_posts() ) :
                    while ( $flats->have_posts() ) :
                        ...
                        <div class="single_layout-bl">
                            <a href="<?php echo get_permalink(); ?>" class="img_w flex <?php status(); ?>">
                                <?php the_post_thumbnail(); ?>
                            </a>
                            <h3>
                                <?php echo __(' [:ua]2-кімнатна квартира
                                [:ru]2-комнатная квартира
                                [:en]2 room[:]' .' (' . esc_html( get_the_title()) . ')' ); ?>
                            </h3>
                            <div class="info">
                                <div class="inner_info flex flex-sb">
                                    <p>
                                        <?php echo __('[:ua]Загальна:[:ru]Общая:[:en]Total:[:]') . ' ';
                                        the_field('total_area'); echo ' '. __('[:ua]м[:ru]м[:en]m[:]'); ?><sup>2</sup>
                                    </p>
                                    <p>|</p>
                                    <p>
                                        <?php echo __('[:ua]Житлова:[:ru]Жилая:[:en]Dwelling:[:]') . ' ';
                                        the_field('dwelling_place'); echo ' '. __('[:ua]м[:ru]м[:en]m[:]'); ?><sup>2</sup>
                                    </p>
                                </div>
                                <div class="location flex">
                                    <p><?php the_field('section');
                                        echo ' '. __('[:ua]секція[:ru]секция[:en]section[:]'); ?>
                                    </p>
                                    <p>-</p>
                                    <p><?php the_field('floors');
                                        echo ' '. __('[:ua]поверх[:ru]этаж[:en]floor[:]'); ?>
                                    </p>
                                </div>
                            </div>
                    ...
                    <p class="e_message">
                        <?php echo __('[:ua]Вибачте, таких планувань немає
                        [:ru]Извините, таких планировок нет
                        [:en]Sorry, there are no such plans[:]'); ?>
                    </p>
                <?php endif;
        ...
        <section class="add_descr">
            <div class="cnt txt close">
                <?php echo __('[:ua]Двокімнатне житло в центрі Києва – такий ласий шматочок для кожного громадянина 
                нашої країни. А якщо будинок побудований за найкращими технологіями та з застосуванням 
                енергозберігаючих матеріалів, то він по праву може вважатися надійним житлом на багато років. <br>
                Купити квартиру в такому житловому комплексі можна, це Хенесi Хаус – новий формат якості на ринку 
                нерухомості. Купуючи двокімнатні апартаменти, Ви стаєте щасливим власником оптимального метражу 
                за прийнятною ціною. Інвестуйте вигідно в "Хенесi Хаус" Шевченківського району столиці.
                [:ru]Двухкомнатное жилье в центре Киева – такой лакомый кусочек для каждого гражданина нашей страны. 
                А если дом построен по лучшим технологиям и с применением энергосберегающих материалов, то он по 
                праву может считаться надежным жильем на многие годы. <br>
                Купить квартиру в таком жилищном комплексе можно, это Хенеси Хаус – новый формат качества на 
                рынке недвижимости. Приобретая двухкомнатные апартаменты, Вы становитесь счастливым обладателем 
                оптимального метража по приемлемой цене. Инвестируйте выгодно в “Хенеси Хаус” Шевченковского 
                района столицы.
                [:en]More[:]'); ?>
            </div>
            <div class="btn">
